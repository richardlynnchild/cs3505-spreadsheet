...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// Processes Cell Selection Keys: up, down, right, left and TAB.
        /// </summary>
        /// <param name="KeyData"></param>
        /// <returns></returns>
        public bool MovementKey(ref Message msg, Keys KeyData)
        {
            return drawingPanel.MovementKey(ref msg, KeyData);
        }
        ...
        /// <summary>
        /// Sets the focus of a col/row address
        /// </summary>
        /// <param name="col"></param>
        /// <param name="row"></param>
        public void SetFocus(string name, int row, int col)
        {
            drawingPanel.SetFocus(name, row, col);
        }

        /// <summary>
        /// Returns cells to unfocused
        /// </summary>
        /// <param name="row"></param>
        /// <param name="col"></param>
        public void SetUnfocus(string name, int row, int col)
        {
            drawingPanel.SetUnfocus(name, row, col);
        }
        ...
        private class DrawingPanel : Panel
        {
            ...
            private Dictionary<string, KeyValuePair<int, int>> focused_cells;
            private int _focusCol;
            private int _focusRow;

            private Dictionary<string, KeyValuePair<int, int>> unfocused_cells;
            private int _unfocusCol;
            private int _unfocusRow;
            ...
            // Current contents (including what's currently being typed) of each cell
            private Dictionary<Address, String> _current_contents;
            ...
            public DrawingPanel(SpreadsheetPanel ss)
            {
                ...
                _ssp = ss;
                _current_contents = new Dictionary<Address, String>();
                focused_cells = new Dictionary<string, KeyValuePair<int, int>>();
                unfocused_cells = new Dictionary<string, KeyValuePair<int, int>>();
            }
            public bool MovementKey(ref Message msg, Keys keyData)
            {
            ...
            public bool SetFocus(string name, int row, int col)
            {
                if (InvalidAddress(col, row))
                {
                    return false;
                }
                KeyValuePair<int, int> loc = new KeyValuePair<int, int>(row, col);
                if (focused_cells.ContainsKey(name))
                {
                    focused_cells[name] = loc;
                }
                else
                {
                    focused_cells.Add(name, loc);
                }
                Invalidate();
                return true;
            }

            public bool SetUnfocus(string name, int row, int col)
            {
                if (InvalidAddress(col, row))
                {
                    return false;
                ...
                KeyValuePair<int, int> loc = new KeyValuePair<int, int>(row, col);
                if (unfocused_cells.ContainsKey(name))
                {
                    unfocused_cells[name] = loc;
                }
                else
                {
                    unfocused_cells.Add(name, loc);
                }
                focused_cells.Remove(name);
                Invalidate();
                return true;
            }
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                Brush focus_brush = new SolidBrush(Color.DeepSkyBlue);
                Pen pen = new Pen(brush);
                Pen focus_pen = new Pen(focus_brush);
                ...
                {
                    _unfocusCol = cell.Value.Value;
                    _unfocusRow = cell.Value.Key;
                    if (((_unfocusCol - _firstColumn >= 0) && (_unfocusRow - _firstRow >= 0)))
                    {
                        e.Graphics.FillRectangle(
                            new SolidBrush(Color.White),
                            new Rectangle(LABEL_COL_WIDTH + (_unfocusCol - _firstColumn) * DATA_COL_WIDTH + 1,
                                          LABEL_ROW_HEIGHT + (_unfocusRow - _firstRow) * DATA_ROW_HEIGHT + 1,
                                          DATA_COL_WIDTH - 2,
                                          DATA_ROW_HEIGHT - 2));
                    }
                }
                ...
                //Do something like above for "focused selection"
                foreach (KeyValuePair<string, KeyValuePair<int, int>> cell in focused_cells)
                {
                    _focusCol = cell.Value.Value;
                    _focusRow = cell.Value.Key;
                    if ((_focusCol - _firstColumn >= 0) && (_focusRow - _firstRow >= 0))
                    {
                        e.Graphics.FillRectangle(
                            new SolidBrush(Color.LightCyan),
                            new Rectangle(LABEL_COL_WIDTH + (_focusCol - _firstColumn) * DATA_COL_WIDTH + 1,
                                          LABEL_ROW_HEIGHT + (_focusRow - _firstRow) * DATA_ROW_HEIGHT + 1,
                                          DATA_COL_WIDTH - 2,
                                          DATA_ROW_HEIGHT - 2));
                    }
                }
                ...
                //idea: maybe have something like _values that contains the current contents of each cell -- values and anything currently being edited
                //then have a foreach like above to draw them all in
